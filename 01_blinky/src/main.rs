...
pub extern "C" fn _start_kernel() -> ! {
    ...
    // To use the pointer.offset() function that Rust offers, we need to divide
    // the raw byte offsets by 4. This is because the offset() function
    // multiplies the arg by the size of the calling pointer type. Since we use
    // type *mut u32, this means the offset is multiplied by sizeof(u32)=4 bytes
